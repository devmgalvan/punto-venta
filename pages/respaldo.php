...
function fetchTableDumpSql($table, $fp = 0)
{
    $tabledump = "--\n";
    if (!$hay_Zlib) {
        fwrite($fp, $tabledump);
    ...
    $tabledump = queryFirst("SHOW CREATE TABLE $table");
    stripBackticks($tabledump['Create Table']);
    ...
    while ($row = fetchArray($rows, DBARRAY_NUM)) {
...
if (!$error) {
    ...
    if (!$result) {
        ...
        if (!$filehandle) {
            ...
            while ($currow = fetchArray($result, DBARRAY_NUM)) {
                fetchTableDumpSql($currow[0], $filehandle);
                fwrite($filehandle, "\n");
                if (!$hay_Zlib) {
                    fwrite($filehandle, "\n");
                } else {
